...
2. スライダーで出力値を決めます。
3. `開始`を押します。画面内部でManual設定、ARM、STARTを順番に実行します。
4. 通常停止は`停止`、緊急時は`緊急停止`を使用します。

同時に有効になるサーボは必ず1チャンネルだけです。選択していない2チャンネルはPCA9685 Full OFFになり、推進出力は常に0、D10リレーはLOWのままです。画面には接続状態と実際のサーボパルスだけを表示し、姿勢補助、方位保持、自動航行、ウェイポイント、センサ詳細、JSONは表示しません。各APIと自動制御の内部実装は、後の統合試験用に保持しています。
