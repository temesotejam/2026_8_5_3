...
production_control::Controller competitionController{}; production_control::CommandIngress competitionIngress{}; production_control::Output competitionOutput{}; uint32_t lastCompetitionOutputTxMs=0,competitionPwmWrites=0,competitionPwmErrors=0; uint64_t lastCompetitionActuatorUs=0,lastVescControlUs=0; uint16_t competitionLeftPulseUs=0,competitionRightPulseUs=0,competitionRearPulseUs=0; float vescTargetDuty=0.0f,vescAppliedDuty=0.0f; production_control::DutyRamp vescRamp(kVescMaxDuty,kVescRampRiseSeconds,kVescRampFallSeconds); production_control::ServoMapper competitionLeftServo(production_control::ServoTuning(kCompetitionServoMinUs,kCompetitionServoNeutralUs,kCompetitionServoMaxUs,kCompetitionServoRateUsPerSecond,kCompetitionLeftReversed)),competitionRightServo(production_control::ServoTuning(kCompetitionServoMinUs,kCompetitionServoNeutralUs,kCompetitionServoMaxUs,kCompetitionServoRateUsPerSecond,kCompetitionRightReversed)),competitionRearServo(production_control::ServoTuning(kCompetitionServoMinUs,kCompetitionServoNeutralUs,kCompetitionServoMaxUs,kCompetitionServoRateUsPerSecond,kCompetitionRearReversed));
...
void setup(){Serial.begin(115200);
  production_control::Config competitionConfig{};competitionConfig.kpPitch=kCompetitionKpPitch;competitionConfig.kdPitch=kCompetitionKdPitch;competitionConfig.kpRoll=kCompetitionKpRoll;competitionConfig.kdRoll=kCompetitionKdRoll;competitionConfig.kpHeight=kCompetitionKpHeight;competitionConfig.kpYaw=kCompetitionKpYaw;competitionConfig.kdYaw=kCompetitionKdYaw;competitionConfig.targetHeightM=kCompetitionTargetHeightM;competitionConfig.autoPropulsion=kCompetitionAutoPropulsion;competitionConfig.waypointReachM=kCompetitionWaypointReachM;competitionConfig.losLookaheadM=kCompetitionLosLookaheadM;competitionConfig.lowVoltageV=kLowVoltageTripV;competitionConfig.criticalVoltageV=kCriticalVoltageTripV;competitionConfig.overCurrentA=kOverCurrentTripA;competitionConfig.criticalCurrentA=kCriticalCurrentTripA;competitionConfig.stallCurrentA=kStallCurrentA;competitionConfig.stallRpm=kStallRpm;competitionConfig.cavitationRpm=kCavitationRpm;competitionConfig.cavitationCurrentA=kCavitationCurrentA;competitionConfig.cavitationSpeedMps=kCavitationSpeedMps;competitionConfig.powerStaleUs=kPowerStateStaleUs;competitionConfig.rpmStaleUs=kRpmStateStaleUs;competitionConfig.vescStaleUs=kVescStateStaleUs;competitionController.setConfig(competitionConfig);
  delay(100);bootId=esp_random();Wire.begin(kPeripheralSdaPin,kPeripheralSclPin,kPeripheralI2cHz);Wire.setTimeOut(20);scan();pcaReady=pca.begin();safeOutputs();ina.profile=InaProfile::Balanced;inaReady=ina.begin();as5600Ready=as5600.begin();tofReady=configureTof(experiment_config::kTofProfile);imu.begin();vescUart.begin(kVescUartBaud,SERIAL_8N1,kVescRxPin,kVescTxPin);linkUart.begin(kLinkBaud,SERIAL_8N1,kLinkRxPin,kLinkTxPin);xTaskCreatePinnedToCore(linkTxTask,"LinkTx",4096,nullptr,kLinkTxTaskPriority,nullptr,0);// BNO task reads reports and publishes the direct Rotation Vector state.
