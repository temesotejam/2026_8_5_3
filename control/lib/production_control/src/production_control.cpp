...
Output Controller::step(const SensorInput& input){
  ...
  if(output.propulsionPrelimit>.02f){
    ...
    if(!fresh(input.rpmValid,input.rpmUs,input.nowUs,config_.rpmStaleUs)){safe(output,StopReason::RpmStale,SafetyRequest::Fault);return output;}
    ...
    const bool stalled=output.propulsionPrelimit>=config_.stallCommand&&std::fabs(input.mechanicalRpm)<config_.stallRpm&&std::fabs(input.currentA)>config_.stallCurrentA;
    ...
    const bool cavitating=std::fabs(input.mechanicalRpm)>config_.cavitationRpm&&std::fabs(input.currentA)<config_.cavitationCurrentA&&input.groundSpeedMps<config_.cavitationSpeedMps;
...
const char* reasonName(StopReason value){switch(value){case StopReason::None:return"NONE";case StopReason::Stop:return"STOP";case StopReason::EStop:return"E_STOP";case StopReason::Heartbeat:return"HEARTBEAT";case StopReason::ManualTimeout:return"MANUAL_TIMEOUT";case StopReason::ImuInvalid:return"IMU_INVALID";case StopReason::ImuStale:return"IMU_STALE";case StopReason::TofInvalid:return"TOF_INVALID";case StopReason::TofStale:return"TOF_STALE";case StopReason::GnssInvalid:return"GNSS_INVALID";case StopReason::GnssStale:return"GNSS_STALE";case StopReason::VescFault:return"VESC_FAULT";case StopReason::NonFinite:return"NONFINITE";case StopReason::FinalWaypoint:return"FINAL_WAYPOINT";case StopReason::PowerInvalid:return"POWER_INVALID";case StopReason::PowerStale:return"POWER_STALE";case StopReason::LowVoltage:return"LOW_VOLTAGE";case StopReason::OverCurrent:return"OVER_CURRENT";case StopReason::MotorStall:return"MOTOR_STALL";case StopReason::AttitudeDanger:return"ATTITUDE_DANGER";case StopReason::VescStale:return"VESC_STALE";case StopReason::RpmStale:return"RPM_STALE";default:return"CAVITATION";}}
